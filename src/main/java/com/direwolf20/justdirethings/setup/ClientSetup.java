...
public class ClientSetup {
	public static void init(final FMLClientSetupEvent event) {
		...
		event.enqueueWork(() -> {
			...
			ItemBlockRenderTypes.setRenderLayer(Registration.UNSTABLE_PORTAL_FLUID_BLOCK.get(),
			...
			ItemBlockRenderTypes.setRenderLayer(Registration.UNSTABLE_PORTAL_FLUID_SOURCE.get(),
					RenderType.translucent());
			ItemBlockRenderTypes.setRenderLayer(Registration.UNSTABLE_PORTAL_FLUID_FLOWING.get(),
					RenderType.translucent());
	...
	public static void registerEnabledToolTextures(Item tool) {
		if (tool instanceof ToggleableItem toggleableItem) {
			ItemProperties.register(tool, new ResourceLocation(JustDireThings.MODID, "enabled"),
					(stack, level, living, id) -> {
						if (stack.getItem() instanceof PocketGenerator) {
							...
							if (energyStorage.getEnergyStored() > 0)
								return 1.0f;
							if (!(NBTHelpers.getIntValue(stack, PocketGenerator.COUNTER) > 0))
								return 0.0f;
							return 1.0f;
						} else
	...
	public static void registerItemColors(RegisterColorHandlersEvent.Item event) {
		...
		event.register(bucketColors, Registration.REFINED_T2_FLUID_BUCKET.get(),
				Registration.REFINED_T3_FLUID_BUCKET.get(), Registration.REFINED_T4_FLUID_BUCKET.get(),
				Registration.UNREFINED_T2_FLUID_BUCKET.get(), Registration.UNREFINED_T3_FLUID_BUCKET.get(),
				Registration.UNREFINED_T4_FLUID_BUCKET.get(), Registration.PORTAL_FLUID_BUCKET.get(),
				Registration.UNSTABLE_PORTAL_FLUID_BUCKET.get(), Registration.TIME_FLUID_BUCKET.get(),
				Registration.XP_FLUID_BUCKET.get(), Registration.POLYMORPHIC_FLUID_BUCKET.get());
	}
