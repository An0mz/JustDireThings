 ...
 *
 * <p>
 * Caches {@link LazyOptional}{@code <IEnergyStorage>} lookups from neighboring
 * block entities. When the cached optional is invalidated by the neighbor (e.g.
 * the block entity is removed or replaced), the entry is automatically evicted
 * so the next call transparently re-queries — saving redundant
 * {@link BlockEntity#getCapability} calls every tick.
 *
 * <p>
 * Usage:
 *
...
public class BlockEnergyCache {
	...
	/**
	 * Key composed of an immutable {@link BlockPos} and the query {@link Direction}
	 * (nullable for isotropic). Records provide structural equals/hashCode
	 * automatically.
	 */
	...
	/**
	 * Returns the {@link IEnergyStorage} at {@code pos} queried from {@code side},
	 * using a cached {@link LazyOptional} when still valid.
	 *
	 * @param level
	 *            the level to query
	 * @param pos
	 ...
	 * @param side
	 *            the side to query from ({@code null} = isotropic / side-agnostic
	 *            query)
	...
	/**
	 * Eagerly invalidate all cached entries for a specific position (across all
	 * sides). Use when you know a particular block has changed.
	 */
	...
	/**
	 * Clear the entire cache. Call from {@code invalidateCaps()} or when the
	 * tracked area is rescanned.
	 */
