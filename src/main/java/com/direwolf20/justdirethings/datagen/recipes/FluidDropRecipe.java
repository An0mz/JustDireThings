...
public class FluidDropRecipe implements CraftingRecipe {
    ...
    public static class Serializer implements RecipeSerializer<FluidDropRecipe> {
        private static final Codec<Item> ITEM_CODEC = ResourceLocation.CODEC.xmap(
                rl -> ForgeRegistries.ITEMS.getValue(rl),
                item -> ForgeRegistries.ITEMS.getKey(item)
        );
        private static final Codec<FluidDropRecipe> CODEC = RecordCodecBuilder.create(
        ...
        public FluidDropRecipe fromJson(ResourceLocation id, JsonObject json) {
            BlockState inputState = BlockState.CODEC.parse(JsonOps.INSTANCE, json.get("input"))
                    .resultOrPartial(e -> {}).orElse(net.minecraft.world.level.block.Blocks.AIR.defaultBlockState());
            BlockState outputState = BlockState.CODEC.parse(JsonOps.INSTANCE, json.get("output"))
                    .resultOrPartial(e -> {}).orElse(net.minecraft.world.level.block.Blocks.AIR.defaultBlockState());
