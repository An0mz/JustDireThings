...
public class BlockEvents {
	...
	public static void BlockToolModificationEvent(BlockEvent.BlockToolModificationEvent event) {
		...
		if (event.getToolAction().equals(ToolActions.HOE_TILL)
				&& heldItem.getItem() instanceof ToggleableItem toggleableItem) {
			if (heldItem.getItem() instanceof FerricoreHoe && toggleableItem.getEnabled(heldItem)) {
				BlockState modifiedState = event.getState().getBlock().getToolModifiedState(event.getState(),
						event.getContext(), event.getToolAction(), true);
	...
	public static void onFurnaceFuelBurnTime(FurnaceFuelBurnTimeEvent event) {
		...
		if (item == Registration.Coal_T1.get())
			...
			if (block == Registration.CoalBlock_T1.get())
				event.setBurnTime(48000);
			else if (block == Registration.CoalBlock_T2.get())
				event.setBurnTime(144000);
			else if (block == Registration.CoalBlock_T3.get())
				event.setBurnTime(432000);
			else if (block == Registration.CoalBlock_T4.get())
