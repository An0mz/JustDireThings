...
public class CreatureCatcherEntity extends ThrowableItemProjectile {
	private static final EntityDataAccessor<Boolean> HAS_HIT = SynchedEntityData.defineId(CreatureCatcherEntity.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> CAPTURING = SynchedEntityData.defineId(CreatureCatcherEntity.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> SHRINKING_TIME = SynchedEntityData
			.defineId(CreatureCatcherEntity.class, EntityDataSerializers.INT);
	private static final EntityDataAccessor<ItemStack> RETURN_ITEM_STACK = SynchedEntityData
			.defineId(CreatureCatcherEntity.class, EntityDataSerializers.ITEM_STACK);
	private static final EntityDataAccessor<Vector3f> ENTITY_POSITION = SynchedEntityData
			.defineId(CreatureCatcherEntity.class, EntityDataSerializers.VECTOR3);
	private static final EntityDataAccessor<Float> ENTITY_BODY_ROT = SynchedEntityData
			.defineId(CreatureCatcherEntity.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> ENTITY_HEAD_ROT = SynchedEntityData
			.defineId(CreatureCatcherEntity.class, EntityDataSerializers.FLOAT);
	...
	protected void onHitEntity(EntityHitResult pResult) {
		...
		if (level().isClientSide || CreatureCatcher.hasEntity(itemStack))
			return;
		Entity entity = pResult.getEntity();
		if (entity instanceof Mob mob && canCapture(mob)) {
			this.entityData.set(HAS_HIT, true);
			this.entityData.set(CAPTURING, true);
			this.entityData.set(ENTITY_POSITION,
					new Vector3f((float) mob.position().x, (float) mob.position().y, (float) mob.position().z));
	...
	public static Mob getEntityFromItemStack(ItemStack itemStack, Level level) {
		...
		if (tag.isEmpty() || !tag.contains("entityType"))
			return null;
	...
	public void readAdditionalSaveData(CompoundTag pCompound) {
		...
		Vector3f position = new Vector3f(pCompound.getFloat("EntityPosX"), pCompound.getFloat("EntityPosY"),
