...
public class ParadoxEntity extends Entity {
    ...
    private static final EntityDataAccessor<Integer> REQUIRED_CONSUMPTION =
            SynchedEntityData.defineId(ParadoxEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> CONSUMPTION =
            SynchedEntityData.defineId(ParadoxEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> RADIUS =
            SynchedEntityData.defineId(ParadoxEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Float> SHRINK_SCALE =
            SynchedEntityData.defineId(ParadoxEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> TARGET_RADIUS =
            SynchedEntityData.defineId(ParadoxEntity.class, EntityDataSerializers.INT);
    ...
    private void handleBlockAbsorption(int currentRadius) {
        ...
        while (iterator.hasNext()) {
            ...
            if (blockStack.isEmpty()) blockStack = new ItemStack(Blocks.STONE);
            Vec3 target = position().add(0, 0.5, 0);
            ParadoxParticleData data = new ParadoxParticleData(blockStack, target.x, target.y, target.z);

            float spawnChance = 1;
            if (blocksToAbsorb.size() > maxBlocksForPerf * 2) spawnChance = 0.25f;
            else if (blocksToAbsorb.size() > maxBlocksForPerf) spawnChance = 0.50f;
            ...
            if (timeLeft <= 0) {
                int particles = 8;
                if (blocksToAbsorb.size() > maxBlocksForPerf * 2) particles = 3;
                else if (blocksToAbsorb.size() > maxBlocksForPerf) particles = 5;
    ...
    private boolean isValidEntity(Entity entity) {
        ...
        if (entity.getType().is(JustDireEntityTags.PARADOX_ABSORB_DENY)) return false;
        return true;
    }
    ...
    public boolean isBlockValid(BlockPos blockPos) {
        ...
        if (blockState.is(JustDireBlockTags.PARADOX_ABSORB_DENY)) return false;
        if (blocksToAbsorb.containsKey(blockPos)) return false;
    ...
    public void setRadius(int radius) { this.entityData.set(RADIUS, Math.min(radius, maxRadius)); }
    public int getGrowthTicks() { return this.entityData.get(GROWTH_TICKS); }
    public void setGrowthTicks(int growthTicks) { this.entityData.set(GROWTH_TICKS, growthTicks); }
    public int getTargetRadius() { return this.entityData.get(TARGET_RADIUS); }
    public void setTargetRadius(int radius) { this.entityData.set(TARGET_RADIUS, radius); setGrowthTicks(0); }
    public float getShrinkScale() { return this.entityData.get(SHRINK_SCALE); }
    public void setShrinkScale(float scale) { this.entityData.set(SHRINK_SCALE, scale); }
    public int getRequiredConsumption() { return this.entityData.get(REQUIRED_CONSUMPTION); }
    public int getConsumed() { return this.entityData.get(CONSUMPTION); }

    @Override
    protected void readAdditionalSaveData(CompoundTag tag) {
        if (tag.contains("radius")) this.entityData.set(RADIUS, tag.getInt("radius"));
        if (tag.contains("targetRadius")) this.entityData.set(TARGET_RADIUS, tag.getInt("targetRadius"));
        if (tag.contains("radiusGrowthTimer")) this.radiusGrowthTimer = tag.getInt("radiusGrowthTimer");
        if (tag.contains("shrinkScale")) this.entityData.set(SHRINK_SCALE, tag.getFloat("shrinkScale"));
    }
