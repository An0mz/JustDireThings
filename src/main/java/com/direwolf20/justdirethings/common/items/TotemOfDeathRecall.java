			...
			if (usedDuration >= 20) { // 60 ticks = 3 seconds
				...
				if (!world.isClientSide) {
					...
					if (tag != null) {
						...
						if (targetLevel != null) {
							player.teleportTo(targetLevel, position.x(), position.y(), position.z(), new HashSet<>(),
									player.getYRot(), player.getXRot());
							stack.shrink(1);
							world.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ITEM_BREAK,
	...
	public void appendHoverText(ItemStack stack, @javax.annotation.Nullable Level level, List<Component> tooltip,
		...
		if (boundPos != null) {
			tooltip.add(Component.translatable("justdirethings.boundto",
					I18n.get(boundPos.dimension().location().getPath()), boundPos.toVec3ShortString())
