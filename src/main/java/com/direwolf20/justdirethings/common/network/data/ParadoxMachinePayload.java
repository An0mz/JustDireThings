...
public record ParadoxMachinePayload(int action, boolean renderParadox, int targetType, double xRadius, double yRadius,
	...
	public ParadoxMachinePayload(final FriendlyByteBuf buffer) {
		this(buffer.readInt(), buffer.readBoolean(), buffer.readInt(), buffer.readDouble(), buffer.readDouble(),
				buffer.readDouble(), buffer.readInt(), buffer.readInt(), buffer.readInt());
	}
