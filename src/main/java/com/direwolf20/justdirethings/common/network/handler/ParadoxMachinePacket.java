		...
		ctx.get().enqueueWork(() -> {
			...
			if (container instanceof BaseMachineContainer baseMachineContainer
				...
				switch (payload.action()) {
					case 0 -> {
						be.snapshotArea();
						int blocks = be.snapshotData.contains("blocks")
								? be.snapshotData.getList("blocks", 10).size()
								: 0;
						int entities = be.snapshotData.contains("entities")
								? be.snapshotData.getList("entities", 10).size()
								: 0;
						sender.displayClientMessage(
								Component.translatable("justdirethings.paradox.snapshot_accepted", blocks, entities),
								true);
					}
