...
public class FuelCanisterScreen extends BaseScreen<FuelCanisterContainer> {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		guiGraphics.drawString(font, msg, this.getGuiLeft() + this.imageWidth / 2 - font.width(msg) / 2,
				getGuiTop() + 5, Color.DARK_GRAY.getRGB(), false);
	...
	protected void renderTooltip(GuiGraphics pGuiGraphics, int pX, int pY) {
		if (this.menu.getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
			...
			int fuelPerPiece = ForgeHooks.getBurnTime(itemstack, RecipeType.SMELTING);
			List<Component> tooltip = this.getTooltipFromContainerItem(itemstack);
			if (container.handler.isItemValid(0, itemstack)) {
				if (hasShiftDown()) {
					tooltip.add(Component
							.translatable("justdirethings.fuelcanisteramt", MagicHelpers.formatted(fuelPerPiece))
							.withStyle(ChatFormatting.AQUA));
					tooltip.add(Component
							.translatable("justdirethings.fuelcanisteramtstack",
									MagicHelpers.formatted(fuelPerPiece * itemstack.getCount()))
							.withStyle(ChatFormatting.AQUA));
				} else {
					tooltip.add(
							Component
									.translatable("justdirethings.fuelcanisteritemsamt",
											MagicHelpers.formatted((float) fuelPerPiece / 200))
									.withStyle(ChatFormatting.AQUA));
					tooltip.add(Component
							.translatable("justdirethings.fuelcanisteritemsamtstack",
									MagicHelpers.formatted((float) (fuelPerPiece * itemstack.getCount()) / 200))
							.withStyle(ChatFormatting.AQUA));
				}
			}
