...
public class DropperT2Screen extends BaseMachineScreen<DropperT2Container> {
	...
	public void addFilterButtons() {
		addRenderableWidget(
				ToggleButtonFactory.COMPARENBTBUTTON(getGuiLeft() + 8, topSectionTop + 62, filterData.compareNBT, b -> {
	...
	public void addAreaButtons() {
		...
		valueButtonsList.add(new ValueButtons(getGuiLeft() + 25, topSectionTop + 12, xOffset,
				-ItemCollectorBE.maxOffset, ItemCollectorBE.maxOffset, font, (button, value) -> {
					xOffset = value;
					saveSettings();
				}));

		valueButtonsList.add(new ValueButtons(getGuiLeft() + 75, topSectionTop + 12, yOffset,
				-ItemCollectorBE.maxOffset, ItemCollectorBE.maxOffset, font, (button, value) -> {
					yOffset = value;
					saveSettings();
				}));

		valueButtonsList.add(new ValueButtons(getGuiLeft() + 125, topSectionTop + 12, zOffset,
				-ItemCollectorBE.maxOffset, ItemCollectorBE.maxOffset, font, (button, value) -> {
					zOffset = value;
					saveSettings();
				}));
	...
	public void init() {
		...
		addRenderableWidget(new NumberButton(getGuiLeft() + 20, topSectionTop + 41, 24, 12, dropCount, 1, 64,
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		if (baseMachineBE instanceof AreaAffectingBE) {
			...
			guiGraphics.drawString(this.font, Component.literal("Off"), xStart - 4, topSectionTop - getGuiTop() + 14,
					4210752, false);
