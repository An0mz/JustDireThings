...
public class ParadoxMachineScreen extends BaseMachineScreen<ParadoxMachineContainer> {
	...
	public void addSnapshotButton() {
		addRenderableWidget(ToggleButtonFactory.SNAPSHOT_AREA_BUTTON(getGuiLeft() + 116, topSectionTop + 62,
				b -> PacketHandler.CHANNEL.sendToServer(new ParadoxMachinePayload(0, false, 0, 0, 0, 0, 0, 0, 0))));
	}

	public void addRenderButton() {
		addRenderableWidget(
				ToggleButtonFactory.RENDERPARADOXBUTTON(getGuiLeft() + 98, topSectionTop + 62, renderParadox, b -> {
	...
	public void addTargetButton() {
		addRenderableWidget(
				ToggleButtonFactory.PARADOXTARGETBUTTON(getGuiLeft() + 56, topSectionTop + 38, targetType, b -> {
	...
	public void saveSettings() {
		super.saveSettings();
		PacketHandler.CHANNEL.sendToServer(new ParadoxMachinePayload(1, renderParadox, targetType, 0, 0, 0, 0, 0, 0));
	}
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		if (baseMachineBE instanceof ParadoxMachineBE paradoxMachineBE) {
			guiGraphics.blit(PARADOXBAR, topSectionLeft + topSectionWidth - 18 - 5, topSectionTop + 5, 0, 0, 18, 72, 36,
					72);
			...
			if (maxEnergy > 0 && paradoxMachineBE.paradoxEnergy > 0) {
				...
				int color = Color.HSBtoRGB((time % 10800L) / 10800.0f, 1.0f, 1.0f);

				int red = (color >> 16) & 0xFF;
				int green = (color >> 8) & 0xFF;
				int blue = color & 0xFF;

				guiGraphics.setColor(red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f);
				guiGraphics.blit(PARADOXBAR, topSectionLeft + topSectionWidth - 18 - 5 + 1,
						topSectionTop + getEnergyBarOffset() + 72 - 2 - (int) remaining, 19, 69 - remaining, 17,
						(int) remaining + 1, 36, 72);
			...
			if (MiscTools.inBounds(topSectionLeft + topSectionWidth - 18 - 5, topSectionTop + 5, 18, 72, pX, pY)) {
				...
				pGuiGraphics.renderTooltip(font,
						Language.getInstance()
								.getVisualOrder(Arrays.asList(Component.translatable("justdirethings.paradoxenergy",
										energyFormatted, paradoxMachineBE.getMaxParadoxEnergy()))),
						pX, pY);
			}
		}
	}
	...
	public void powerBarTooltip(GuiGraphics pGuiGraphics, int pX, int pY) {
		if (baseMachineBE instanceof ParadoxMachineBE paradoxMachineBE
				&& baseMachineBE instanceof PoweredMachineBE poweredMachineBE) {
											...
											.getVisualOrder(
													Arrays.asList(
															Component.translatable("justdirethings.screen.energy",
																	...
																	MagicHelpers.formatted(
																			poweredMachineBE.getMaxEnergy())),
															Component.translatable(
																	"justdirethings.screen.paradoxenergycost",
																	MagicHelpers.formatted(totalCostEnergy)))),
									pX, pY);
				else
					pGuiGraphics
							.renderTooltip(font,
											...
											.getVisualOrder(
													Arrays.asList(
															Component.translatable("justdirethings.screen.energy",
																	...
																	MagicHelpers.withSuffix(
																			poweredMachineBE.getMaxEnergy())),
															Component.translatable(
																	"justdirethings.screen.paradoxenergycost",
																	MagicHelpers.withSuffix(totalCostEnergy)))),
									pX, pY);
			}
		}
	}
	...
	public void fluidBarTooltip(GuiGraphics pGuiGraphics, int pX, int pY) {
		if (baseMachineBE instanceof ParadoxMachineBE paradoxMachineBE
				&& baseMachineBE instanceof FluidMachineBE fluidMachineBE) {
											...
											.getVisualOrder(
													Arrays.asList(
															Component.translatable("justdirethings.screen.fluid",
																	this.container.getFluidStack().getDisplayName(),
																	MagicHelpers
																			.formatted(this.container.getFluidAmount()),
																	MagicHelpers.formatted(fluidMachineBE.getMaxMB())),
															Component.translatable(
																	"justdirethings.screen.paradoxfluidcost",
																	MagicHelpers.formatted(totalCostFluid)))),
									pX, pY);
				else
					pGuiGraphics
							.renderTooltip(font,
											...
											.getVisualOrder(
													Arrays.asList(
															Component.translatable("justdirethings.screen.fluid",
																	this.container.getFluidStack().getDisplayName(),
																	MagicHelpers.withSuffix(
																			this.container.getFluidAmount()),
																	MagicHelpers.withSuffix(fluidMachineBE.getMaxMB())),
															Component.translatable(
																	"justdirethings.screen.paradoxfluidcost",
																	MagicHelpers.withSuffix(totalCostFluid)))),
									pX, pY);
			}
		}
	}
