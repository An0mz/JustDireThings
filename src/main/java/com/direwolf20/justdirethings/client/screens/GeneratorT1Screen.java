...
public class GeneratorT1Screen extends BaseMachineScreen<GeneratorT1Container> {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		if (maxBurn > 0) {
			...
			guiGraphics.blit(JUSTSLOT, getGuiLeft() + 79, getGuiTop() + 30 + 18 - remaining, 18, 36 - remaining, 18,
					remaining + 3);
		}
	}
											...
											.getVisualOrder(
													Arrays.asList(
															Component.translatable(
																	...
																	MagicHelpers
																			.formatted(this.container.getEnergy()),
																	MagicHelpers.formatted(
																			poweredMachineBE.getMaxEnergy())),
															burnRemaining > 0
																	? Component
																			.translatable(
																					"justdirethings.screen.fepertick",
																					MagicHelpers.formatted(
																							generatorBE.fePerTick()))
																	: Component.translatable(
																			"justdirethings.screen.fepertick",
																			MagicHelpers.formatted(0)),
															burnRemaining <= 0
																	? Component.translatable(
																			"justdirethings.screen.no_fuel")
																	: Component.translatable(
																			"justdirethings.screen.burn_time",
																			MagicHelpers
																					.ticksInSeconds(burnRemaining)))),
									pX, pY);
				else
					pGuiGraphics
							.renderTooltip(font,
											...
											.getVisualOrder(
													Arrays.asList(
															Component.translatable(
																	"justdirethings.screen.energy",
																	MagicHelpers
																			.withSuffix(this.container.getEnergy()),
	...
	protected void renderTooltip(GuiGraphics pGuiGraphics, int pX, int pY) {
		if (this.menu.getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
			...
			if (burnTime > 0) {
				...
				tooltip.add(Component.translatable("justdirethings.screen.burnspeedmultiplier", fuelBurnMultiplier)
