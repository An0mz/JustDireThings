...
public class SensorT2Screen extends BaseMachineScreen<SensorT2Container> implements SensorScreenInterface {
	...
	public void init() {
		super.init();
		addRenderableWidget(
				ToggleButtonFactory.SENSORTARGETBUTTON(getGuiLeft() + 26, topSectionTop + 62, senseTarget, b -> {
					senseTarget = ((ToggleButton) b).getTexturePosition();
					saveSettings();
				}));
		addRenderableWidget(ToggleButtonFactory.STRONGWEAKREDSTONEBUTTON(getGuiLeft() + 44, topSectionTop + 62,
				strongSignal ? 1 : 0, b -> {
					strongSignal = ((ToggleButton) b).getTexturePosition() == 1;
					saveSettings();
				}));

		addRenderableWidget(new NumberButton(getGuiLeft() + 122, topSectionTop + 64, 24, 12, senseAmount, 0, 9999,
				Component.translatable("justdirethings.screen.senseamount"), b -> {
					senseAmount = ((NumberButton) b).getValue(); // The value is updated in the mouseClicked method
																	// below
					saveSettings();
				}));
		...
		this.scrollPanel = new BlockStateScrollList(this, topSectionLeft - 95, 90, topSectionTop + 5,
				topSectionTop + topSectionHeight - 10);
	}
	...
	protected boolean hasClickedOutside(double mouseX, double mouseY, int guiLeftIn, int guiTopIn, int mouseButton) {
		if (showBlockStates
				&& MiscTools.inBounds(topSectionLeft - 101, topSectionTop, 100, topSectionHeight, mouseX, mouseY))
			return false;
	...
	public void saveSettings() {
		super.saveSettings();
		PacketHandler.CHANNEL.sendToServer(new SensorPayload(senseTarget, strongSignal, senseAmount, equality));
	}

	public Comparable<?> getValue(Property<?> property) {
		if (!blockStateProperties.containsKey(blockStateSlot))
			return null;
	...
	public void saveBlockStateData(int slot) {
		if (!blockStateProperties.containsKey(slot))
			return;
	...
	public void refreshStateWindow() {
		if (!showBlockStates || blockStateSlot == -1)
			return;
									...
									Language.getInstance()
											.getVisualOrder(
													Arrays.asList(
															...
															Component.translatable("justdirethings.screen.energycost",
																	MagicHelpers
																			.withSuffix(sensorT2BE.getEnergyCost())))),
									pX, pY);
				else
					pGuiGraphics
							.renderTooltip(font,
	...
	protected void renderTooltip(GuiGraphics pGuiGraphics, int pX, int pY) {
		if (hoveredSlot != null && (hoveredSlot instanceof FilterBasicSlot)) {
			if (this.menu.getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
				...
				components.add(Component.translatable("justdirethings.screen.rightclicksettings")
