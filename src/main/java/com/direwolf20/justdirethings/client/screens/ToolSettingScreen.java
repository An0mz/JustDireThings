...
public class ToolSettingScreen extends AbstractContainerScreen<ToolSettingContainer> {
	...
	public void refreshButtons() {
		...
		for (Ability toolAbility : abilities) {
			...
			if (toolAbility.getSettingType() == Ability.SettingType.TOGGLE) {
				...
				button = new GrayscaleButton(buttonsStartX + ((counter / 2) * 18), buttonsStartY + ((counter % 2) * 18),
						16, 16, toolAbility.getIconLocation(), Component.translatable(toolAbility.getLocalization()),
						isActive, (clicked) -> {
				...
				int possibleValues = ((abilityParams.maxSlider - abilityParams.minSlider) / abilityParams.increment)
						+ 2;
				...
				button = ToggleButtonFactory.ABILITYCYCLEBUTTON(buttonsStartX + ((counter / 2) * 18),
						buttonsStartY + ((counter % 2) * 18), toolAbility, buttonPosition, possibleValues,
						(clicked) -> {
				...
				if (abilityParams.minSlider != abilityParams.maxSlider) {
					ForgeSlider slider = new ForgeSlider(buttonsStartX + 20, buttonsStartY - 18, 100, 15,
							Component.translatable(toolAbility.getLocalization()).append(": "), Component.empty(),
							abilityParams.minSlider, abilityParams.maxSlider, currentValue, true) {
						@Override
			...
			if (button != null && tool.getItem() instanceof LeftClickableTool && toolAbility.isBindable()) {
				...
				GrayscaleButton bindingButton = ToggleButtonFactory.KEYBIND_BUTTON(buttonsStartX + 143,
						buttonsStartY - 18, false, (clicked) -> {
							bindingEnabled = true;
							((GrayscaleButton) clicked).toggleActive();
						});
				...
				if (bindingMap.get(button) == null) {
					...
					if (binding.isMouse) {
						...
						pGuiGraphics.renderTooltip(font,
								Language.getInstance()
										.getVisualOrder(Arrays.asList(button.getLocalization(),
												Component.translatable("justdirethings.bound-key", bindingName))),
								pX, pY);
					}
				}
			} else if (renderable instanceof BaseButton button
					&& !button.getLocalization(pX, pY).equals(Component.empty())) {
				if (sliders.containsKey(button) || leftRightClickButtons.containsKey(button))
					pGuiGraphics.renderTooltip(font,
							Language.getInstance()
									.getVisualOrder(Arrays.asList(button.getLocalization(),
											Component.translatable("justdirethings.screen.rightclicksettings"))),
							pX, pY);
				else
	...
	public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers) {
		if (shownAbilityButton != null && leftRightClickButtons.get(shownAbilityButton) != null
		...
		if (btn == 1) {
			for (Renderable renderable : new ArrayList<>(renderables)) { // Create a copy of renderables to iterate over
				if (renderable instanceof Button button
						&& (sliders.containsKey(button) || leftRightClickButtons.containsKey(button)
								|| bindingButtons.containsKey(button) || hideRenderButtons.containsKey(button))
						&& MiscTools.inBounds(button.getX(), button.getY(), button.getWidth(), button.getHeight(), x,
								y)) { // If right click on any button, clear the optional buttons first.
	...
	public boolean showCustomBinding() {
		return (shownAbilityButton != null & leftRightClickButtons.containsKey(shownAbilityButton)
				&& (leftRightClickButtons.get(shownAbilityButton).getTexturePosition() == 2
						|| (leftRightClickButtons.get(shownAbilityButton).getTexturePosition() == 0
								&& buttonToAbilityMap.get(shownAbilityButton)
										.getBindingType() == Ability.BindingType.CUSTOM_ONLY)));
	}
