...
public class PocketGeneratorScreen extends BaseScreen<PocketGeneratorContainer> {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > (leftPos + 7) && mouseX < (leftPos + 7) + 18 && mouseY > (topPos + 7)
				&& mouseY < (topPos + 7) + 73) {
			...
			if (pocketGenerator.getItem() instanceof PocketGenerator pocketGeneratorItem) {
				feBurnPerTick = pocketGeneratorItem.getFePerFuelTick()
						* pocketGeneratorItem.getBurnSpeedMultiplier(pocketGenerator);
			}
					...
					Language.getInstance().getVisualOrder(Arrays.asList(
							...
							counter <= 0
									...
									: Component.translatable("justdirethings.screen.burn_time",
											MagicHelpers.ticksInSeconds(counter)),
							counter > 0
									? Component.translatable("justdirethings.screen.fepertick",
											MagicHelpers.formatted(feBurnPerTick))
									: Component.translatable("justdirethings.screen.fepertick",
