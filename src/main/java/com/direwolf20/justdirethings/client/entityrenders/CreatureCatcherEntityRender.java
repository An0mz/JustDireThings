...
public class CreatureCatcherEntityRender extends ThrownItemRenderer<CreatureCatcherEntity> {
	...
	@Override
	public void render(CreatureCatcherEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack,
			MultiBufferSource pBuffer, int pPackedLight) {
		if (pEntity.tickCount >= 2
				|| !(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(pEntity) < 12.25)) {
			pPoseStack.pushPose();
			...
			if (!capturing && pEntity.renderTick > 0) {
				this.itemRenderer.renderStatic(new ItemStack(Registration.CreatureCatcher.get()),
						ItemDisplayContext.GROUND, pPackedLight, OverlayTexture.NO_OVERLAY, pPoseStack, pBuffer,
						pEntity.level(), pEntity.getId());
			} else {
				this.itemRenderer.renderStatic(pEntity.getItem(), ItemDisplayContext.GROUND, pPackedLight,
						OverlayTexture.NO_OVERLAY, pPoseStack, pBuffer, pEntity.level(), pEntity.getId());
			}
			pPoseStack.popPose();
			...
			Vector3f entityPosition = new Vector3f((float) pEntity.getX(),
					(float) pEntity.getY() + (float) (pEntity.getBoundingBox().getYsize() / 2), (float) pEntity.getZ());
			Vector3f originalPosition;
			float fraction; // Shrink Factor ranging from 0.0 being full-size, to 1.0 being shrunken to
							// nothing
			...
			if (capturing) { // Capturing
				...
				fraction = (pEntity.getAnimationTicks() - interpolatedShrinkingTime)
						/ (float) pEntity.getAnimationTicks();
			...
			pPoseStack.translate(interpolatedPosition.x() - pEntity.getX(), interpolatedPosition.y() - pEntity.getY(),
					interpolatedPosition.z() - pEntity.getZ());
