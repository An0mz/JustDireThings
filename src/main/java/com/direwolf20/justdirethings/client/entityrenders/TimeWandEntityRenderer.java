...
public class TimeWandEntityRenderer extends EntityRenderer<TimeWandEntity> {
	...
	@Override
	public void render(TimeWandEntity entity, float yaw, float partialTick, PoseStack poseStack,
		...
		drawText(poseStack, buffer, timeRate, new Vector3f(-0.51F, 0.39f, -pad), Axis.YP.rotationDegrees(-90F), white);
		drawText(poseStack, buffer, timeRate, new Vector3f(-pad, 1.01F, 0.5f - 0.39f), Axis.XP.rotationDegrees(90F),
				white);
		drawText(poseStack, buffer, timeRate, new Vector3f(-pad, -0.01F, -0.5f + 0.39f), Axis.XP.rotationDegrees(-90F),
				white);

		drawText(poseStack, buffer, timeRemains, new Vector3f(-pad, 0.69f, 0.51F), Axis.YP.rotationDegrees(0), white);
		drawText(poseStack, buffer, timeRemains, new Vector3f(pad, 0.69f, -0.51F), Axis.YP.rotationDegrees(180F),
				white);
		drawText(poseStack, buffer, timeRemains, new Vector3f(0.51F, 0.69f, pad), Axis.YP.rotationDegrees(90F), white);
		drawText(poseStack, buffer, timeRemains, new Vector3f(-0.51F, 0.69f, -pad), Axis.YP.rotationDegrees(-90F),
				white);
		drawText(poseStack, buffer, timeRemains, new Vector3f(-pad, 1.01F, 0.5f - 0.69f), Axis.XP.rotationDegrees(90F),
				white);
		drawText(poseStack, buffer, timeRemains, new Vector3f(-pad, -0.01F, -0.5f + 0.69f),
				Axis.XP.rotationDegrees(-90F), white);

		poseStack.popPose();
	}

	private void renderProgressBarOnSide(PoseStack poseStack, MultiBufferSource buffer, float yStart, float progress,
			float r, float g, float b, float a, int packedLight) {
		float barWidth = 0.8f;
		float barHeight = 0.1f;
		float barProgress = barWidth * progress;

		renderBarFace(poseStack, buffer, -0.4f, yStart, 0.5f, barProgress, barHeight, r, g, b, a,
		...
		renderBarFace(poseStack, buffer, -0.4f, yStart, 0.5f, barProgress, barHeight, r, g, b, a,
				new Quaternionf().rotateY((float) -Math.PI / 2.0f));
		renderBarFace(poseStack, buffer, -0.4f, yStart - 0.5f, 1f, barProgress, barHeight, r, g, b, a,
				new Quaternionf().rotateX((float) -Math.PI / 2.0f));
		renderBarFace(poseStack, buffer, -0.4f, yStart - 0.5f, 0f, barProgress, barHeight, r, g, b, a,
				new Quaternionf().rotateX((float) Math.PI / 2.0f));
	}

	private void renderBarFace(PoseStack poseStack, MultiBufferSource buffer, float xStart, float yStart, float zStart,
			float barWidth, float barHeight, float r, float g, float b, float a, Quaternionf rotation) {
		poseStack.pushPose();
		poseStack.mulPose(rotation);
		RenderHelpers.renderBoxSolid(poseStack.last().pose(), buffer,
				new AABB(xStart, yStart, zStart, xStart + barWidth, yStart + barHeight, zStart), r, g, b, a);
		poseStack.popPose();
	}

	private void drawText(PoseStack poseStack, MultiBufferSource source, String text, Vector3f pos, Quaternionf rotate,
			int color) {
		poseStack.pushPose();
		poseStack.translate(pos.x(), pos.y(), pos.z());
		poseStack.scale(0.01F, -0.01F, 0.01F);
		poseStack.mulPose(rotate);
		getFont().drawInBatch(text, 0, 0, -1, false, poseStack.last().pose(), source, Font.DisplayMode.NORMAL, 0,
