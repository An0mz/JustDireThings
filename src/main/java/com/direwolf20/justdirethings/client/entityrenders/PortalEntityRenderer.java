...
public class PortalEntityRenderer extends EntityRenderer<PortalEntity> {
	...
	private static final ResourceLocation PORTAL_SHADER_TEXTURE = ResourceLocation
			.fromNamespaceAndPath("justdirethings", "textures/block/portal_shader.png");
	...
	@Override
	public void render(PortalEntity entity, float yaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer,
			int packedLight) {
		...
		if (facing.getAxis() == Direction.Axis.Z) {
			...
			addTintQuad(borderVc, pose, x0 + BORDER_SIZE, 0f, z0, x1 - BORDER_SIZE, BORDER_SIZE, z1, r, g, b, 0.95f,
					packedLight);
			addTintQuad(borderVc, pose, x0 + BORDER_SIZE, (float) HEIGHT - BORDER_SIZE, z0, x1 - BORDER_SIZE,
					(float) HEIGHT, z1, r, g, b, 0.95f, packedLight);
		} else {
			...
			addTintQuad(borderVc, pose, x0, 0f, z0 + BORDER_SIZE, x1, BORDER_SIZE, z1 - BORDER_SIZE, r, g, b, 0.95f,
					packedLight);
			addTintQuad(borderVc, pose, x0, (float) HEIGHT - BORDER_SIZE, z0 + BORDER_SIZE, x1, (float) HEIGHT,
					z1 - BORDER_SIZE, r, g, b, 0.95f, packedLight);
		}
	...
	private void addTintQuad(VertexConsumer vc, PoseStack.Pose pose, float x0, float y0, float z0, float x1, float y1,
			float z1, float r, float g, float b, float a, int light) {
