...
public class DireVertexConsumerSquished extends VertexConsumerWrapper {
	...
	public DireVertexConsumerSquished(VertexConsumer parent, float minX, float minY, float minZ, float maxX, float maxY,
			float maxZ, Matrix4f matrix4f) {
		super(parent);
		this.minX = minX;
		this.minY = minY;
		this.minZ = minZ;
	...
	public DireVertexConsumerSquished(VertexConsumer parent, float minX, float minY, float minZ, float maxX, float maxY,
			float maxZ, Matrix4f matrix4f, float red, float green, float blue, float alpha) {
		super(parent);
		this.minX = minX;
		this.minY = minY;
		this.minZ = minZ;
			...
			if (bottomUp) {
				...
				if (direction != null) {
					if (!direction.getAxis().equals(Direction.Axis.Y)) {
						...
						float vDistanceToStart = v - minV; // This is static because v is always up and down for the 4
															// sides of a block
						float adjustedUDistance;
						float adjustedU2Distance;
						float adjustedU;
						if (direction.getAxis().equals(Direction.Axis.X)) {
							if (direction.equals(Direction.EAST)) { // These are reverse for Opposite sides, not quite
																	// sure why though
								uDistanceToEnd = maxU - u;
								uDistanceToStart = u - minU;
								adjustedUDistance = uDistanceToEnd * (maxZ); // If we're drawing the X axis, shrink the
																				// Z axis to its appropriate sizes
							...
							if (direction.equals(Direction.NORTH)) {
								uDistanceToEnd = maxU - u;
								uDistanceToStart = u - minU;
								adjustedUDistance = uDistanceToEnd * (maxX); // If we're drawing the Z axis, shrink the
																				// X axis to its appropriate sizes
						...
						float adjustedV = maxV - adjustedVDistance - adjustedV2Distance; // Subtracting because we're
																							// adjusting from the end.
						parent.uv(adjustedU, adjustedV);
						return this;
					} else {
						// When drawing the top/bottom, we do like above, but both U and V are
						// different, so we calculate both
