...
public class RenderHighlight {
	...
	static void renderBlockHighlight(RenderHighlightEvent.Block evt) {
		...
		if (!(toggleableToolStack.getItem() instanceof ToggleableTool toggleableTool))
			return;
		...
		for (BlockPos blockPos : breakBlockPositions) {
			...
			renderHitOutline(evt.getPoseStack(), vertexconsumer2, player, d0, d1, d2, level, blockPos,
					level.getBlockState(blockPos));
		}
	}

	private static void renderHitOutline(PoseStack pPoseStack, VertexConsumer pConsumer, Entity pEntity, double pCamX,
			double pCamY, double pCamZ, Level level, BlockPos pPos, BlockState pState) {
		renderShape(pPoseStack, pConsumer, pState.getShape(level, pPos, CollisionContext.of(pEntity)),
				(double) pPos.getX() - pCamX, (double) pPos.getY() - pCamY, (double) pPos.getZ() - pCamZ, 0.0F, 0.0F,
				0.0F, 0.4F);
	}
	...
	private static void renderShape(PoseStack pPoseStack, VertexConsumer pConsumer, VoxelShape pShape, double pX,
		...
		pShape.forAllEdges((p_234280_, p_234281_, p_234282_, p_234283_, p_234284_, p_234285_) -> {
			...
			f2 /= f3;
			pConsumer
					.vertex(posestack$pose.pose(), (float) (p_234280_ + pX), (float) (p_234281_ + pY),
							(float) (p_234282_ + pZ))
					.color(pRed, pGreen, pBlue, pAlpha).normal(posestack$pose.normal(), f, f1, f2).endVertex();
			pConsumer
					.vertex(posestack$pose.pose(), (float) (p_234283_ + pX), (float) (p_234284_ + pY),
							(float) (p_234285_ + pZ))
					.color(pRed, pGreen, pBlue, pAlpha).normal(posestack$pose.normal(), f, f1, f2).endVertex();
		});
	}
