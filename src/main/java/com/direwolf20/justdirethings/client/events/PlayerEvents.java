...
public class PlayerEvents {
	...
	public static void LeftClickEmpty(PlayerInteractEvent.LeftClickEmpty event) {
		...
		if (itemStack.getItem() instanceof ToggleableTool toggleableTool
			...
			activateAbilities(itemStack, toggleableTool, event.getEntity(), event.getHand(), true, BlockPos.ZERO,
					Direction.DOWN);
		}
	}
	...
	public static void LeftClickBlock(PlayerInteractEvent.LeftClickBlock event) {
		...
		if (itemStack.getItem() instanceof ToggleableTool toggleableTool
				&& itemStack.getItem() instanceof LeftClickableTool && event.getFace() != null) {
			if (event.getAction() == PlayerInteractEvent.LeftClickBlock.Action.START) { // Only start has the 'proper'
																						// direction - this also runs
																						// both client AND server side!
				activateAbilities(itemStack, toggleableTool, event.getEntity(), event.getHand(), false, event.getPos(),
						event.getFace());
			}
		}
		if (itemStack.getItem() instanceof ToggleableTool toggleableTool && toggleableTool.hasAbility(Ability.HAMMER)
				&& event.getFace() != null) {
			doExtraCrumblings(event, itemStack, toggleableTool);
		}
	}

	private static void doExtraCrumblings(PlayerInteractEvent.LeftClickBlock event, ItemStack itemStack,
	...
	private static float incrementDestroyProgress(Level level, BlockState pState, BlockPos pPos, Player player,
		...
		for (BlockPos blockPos : breakBlockPositions) {
			...
			if (level.isClientSide)
				...
				sendDestroyBlockProgress(player.getId() + generatePosHash(blockPos), blockPos, -1,
						(ServerPlayer) player);
		}
		return f;
	}
	...
	public static int generatePosHash(BlockPos blockPos) {
		return (31 * 31 * blockPos.getX()) + (31 * blockPos.getY()) + blockPos.getZ(); // For now this is probably good
																						// enough, will add more
																						// randomness if needed
	}

	public static void sendDestroyBlockProgress(int pBreakerId, BlockPos pPos, int pProgress,
			ServerPlayer serverPlayer) {
		serverPlayer.connection.send(new ClientboundBlockDestructionPacket(pBreakerId, pPos, pProgress));
	}

	private static void activateAbilities(ItemStack itemStack, ToggleableTool toggleableTool, Player player,
			InteractionHand hand, boolean air, BlockPos blockPos, Direction direction) {
		...
		if (!abilities.isEmpty()) {
			...
			if (air) { // Air
				...
				PacketHandler.CHANNEL.sendToServer(
						new LeftClickPayload(0, hand == InteractionHand.MAIN_HAND, BlockPos.ZERO, -1, -1, -1, false)); // Type
																														// 0
																														// ==
																														// air
			} else { // Block - No need for a packet since this will run both client and server side!
						// (See above!)
				UseOnContext useoncontext = new UseOnContext(player.level(), player, hand, itemStack,
						new BlockHitResult(Vec3.atCenterOf(blockPos), direction, blockPos, false));
				toggleableTool.useOnAbility(useoncontext, false);
				// Don't need the below because this actually runs on both client AND server.
				// PacketHandler.CHANNEL.sendToServer(new LeftClickPayload(1, hand ==
				// InteractionHand.MAIN_HAND, blockPos, direction.ordinal(), -1, -1, false));
				// //Type 1 == Block
			}
