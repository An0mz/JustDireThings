...
public class PlayerEvents {
    ...
    /**
     * Client-side player tick: runs passive-tick armor abilities (e.g. walkSpeed, runSpeed, swimSpeed).
     * These abilities call player.moveRelative() which must execute on the client to actually affect movement.
     * The server-side inventoryTick alone is not sufficient because the Minecraft client is authoritative over
    ...
    private static void activateAbilities(ItemStack itemStack, ToggleableTool toggleableTool, Player player, InteractionHand hand, boolean air, BlockPos blockPos, Direction direction) {
        ...
        if (!abilities.isEmpty()) {
            ...
            if (air) { //Air
                ...
                PacketHandler.CHANNEL.sendToServer(new LeftClickPayload(0, hand == InteractionHand.MAIN_HAND, BlockPos.ZERO, -1, -1, -1, false)); //Type 0 == air
                ...
                //PacketHandler.CHANNEL.sendToServer(new LeftClickPayload(1, hand == InteractionHand.MAIN_HAND, blockPos, direction.ordinal(), -1, -1, false)); //Type 1 == Block
            }
