...
public class ParadoxParticleData implements ParticleOptions {
	...
	public String writeToString() {
		return String.format(Locale.ROOT, "%s %.2f %.2f %.2f", this.getType(), this.targetX, this.targetY,
				this.targetZ);
	}
	...
	public static final Deserializer<ParadoxParticleData> DESERIALIZER = new Deserializer<>() {
		@Nonnull
		@Override
		public ParadoxParticleData fromCommand(ParticleType<ParadoxParticleData> type, StringReader reader)
		...
		public ParadoxParticleData fromNetwork(ParticleType<ParadoxParticleData> type, FriendlyByteBuf buffer) {
			return new ParadoxParticleData(buffer.readItem(), buffer.readDouble(), buffer.readDouble(),
					buffer.readDouble());
		}
	};
}
