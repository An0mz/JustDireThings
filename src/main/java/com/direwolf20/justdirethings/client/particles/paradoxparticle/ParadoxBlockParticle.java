...
public class ParadoxBlockParticle extends BreakingItemParticle {
    ...
    public ParadoxBlockParticle(ClientLevel level, double x, double y, double z,
                                double targetX, double targetY, double targetZ, ItemStack itemStack) {
        ...
        if (this.sprite == null) {
            ...
            this.setSprite(Minecraft.getInstance().getItemRenderer()
                    .getModel(fallback, level, (LivingEntity) null, 0).getParticleIcon());
        }
    }
    ...
    public static final ParticleProvider<ParadoxParticleData> FACTORY =
            (data, level, x, y, z, dx, dy, dz) ->
                    new ParadoxBlockParticle(level, x, y, z, data.targetX, data.targetY, data.targetZ, data.getItemStack());
}
