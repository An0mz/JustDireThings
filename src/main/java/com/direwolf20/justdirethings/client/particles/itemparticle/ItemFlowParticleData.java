...
public class ItemFlowParticleData implements ParticleOptions {
	...
	public String writeToString() {
		return String.format(Locale.ROOT, "%s %.2f %.2f %.2f %.2f %.2f %s", this.getType(), this.targetX, this.targetY,
				this.targetZ, this.ticksPerBlock);
	}

	/*
	 * public String getParameters() { return
	 * Registry.PARTICLE_TYPE.getKey(this.getType()) + " " + (new
	 * ItemInput(this.itemStack.getItem().builtInRegistryHolder(),
	 * this.itemStack.getTag())).serialize(); }
	 */
	...
	public static final Deserializer<ItemFlowParticleData> DESERIALIZER = new Deserializer<ItemFlowParticleData>() {
		@Nonnull
		@Override
		public ItemFlowParticleData fromCommand(ParticleType<ItemFlowParticleData> particleTypeIn, StringReader reader)
				throws CommandSyntaxException {
			reader.expect(' ');
			// ItemParser itemparser = (new ItemParser(reader, false)).parse();
			// ItemStack itemstack = (new ItemInput(itemparser.getItem(),
			// itemparser.getNbt())).createItemStack(1, false);
			ItemParser.ItemResult itemparser$itemresult = ItemParser.parseForItem(BuiltInRegistries.ITEM.asLookup(),
					reader);
			ItemStack itemstack = (new ItemInput(itemparser$itemresult.item(), itemparser$itemresult.nbt()))
		...
		@Override
		public ItemFlowParticleData fromNetwork(ParticleType<ItemFlowParticleData> particleTypeIn,
				FriendlyByteBuf buffer) {
			return new ItemFlowParticleData(buffer.readItem(), buffer.readDouble(), buffer.readDouble(),
